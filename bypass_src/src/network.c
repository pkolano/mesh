...
int network_address_to_name( network_address addr, char *name )
{
	...
	#if defined(linux)
		success = gethostbyaddr_r( (char*)&addr, sizeof(addr), AF_INET, &result, buffer, 32768, &h, &herrno );

	#elif defined(__osf__)
		success = gethostbyaddr_r( (const char*)&addr, sizeof(addr), AF_INET, &result, &hdata );
	#else
		h = gethostbyaddr_r( (char*)&addr, sizeof(addr), AF_INET, &result, buffer, 32768, &herrno );
	#endif
...
int network_name_to_address( const char *name, network_address *addr )
{
	...
	#elif defined(__osf__)
		struct hostent_data hdata;
		success = gethostbyname_r( name, &result, &hdata );
	#else
